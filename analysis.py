...
def export_data(df, dept_summary, cat_summary, rec_summary, consol, metadata):
    ...
    # Export summaries with comma delimiter
    if not dept_summary.empty:
        dept_summary.to_csv(OUTPUT_BY_DEPARTMENT, index=False, encoding='utf-8')
        print(f"✓ Exported department summary to: {OUTPUT_BY_DEPARTMENT}")

    if not cat_summary.empty:
        cat_summary.to_csv(OUTPUT_BY_CATEGORY, index=False, encoding='utf-8')
        print(f"✓ Exported category summary to: {OUTPUT_BY_CATEGORY}")

    if not rec_summary.empty:
        rec_summary.to_csv("summary_by_recommendation.csv", index=False, encoding='utf-8')
        print(f"✓ Exported recommendation summary to: summary_by_recommendation.csv")

    if not consol.empty:
        consol.to_csv(OUTPUT_CONSOLIDATION, index=False, encoding='utf-8')
        print(f"✓ Exported consolidation opportunities to: {OUTPUT_CONSOLIDATION}")

    print(f"\n📝 All files exported with standard comma (,) delimiter")
...
def load_enriched_data():
    ...
    if os.path.exists(OUTPUT_ENRICHED):
        df = pd.read_csv(OUTPUT_ENRICHED, encoding='utf-8')
        print(f"\n✓ Loaded enriched data: {len(df)} rows from {OUTPUT_ENRICHED}")
        print(f"   (comma-delimited format)")
        return df
    else:
